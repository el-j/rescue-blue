...
export function SidebarPanels({ t, ctaLabel, ctaBody, formattedSignatureCount, isLoadingSignatures, facts, onCtaClick }: SidebarProps) {
  return (
    <aside className="space-y-6 lg:col-span-4 lg:sticky lg:top-24 lg:self-start lg:max-h-[calc(100vh-7rem)] lg:overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden pr-0.5">
      {/* Primary Action Docket Card */}
      <div className="editorial-card p-6 border-t-4 border-t-blue-600 dark:border-t-blue-500">
        ...
        <a
          ...
          onClick={onCtaClick}
          className="group flex w-full items-center justify-center gap-2 rounded-xl bg-blue-600 hover:bg-blue-700 text-white px-5 py-3.5 text-sm font-bold shadow-md shadow-blue-500/20 transition-all active:scale-[0.99]"
        >
          <span>{ctaLabel}</span>
        ...
        <p className="mt-2 text-center text-[11px] text-[var(--text-faint)]">{t.ctaInfo}</p>
      </div> 
      ...
      <div className="editorial-card p-6">
        ...
        <p className="editorial-numeral text-4xl font-extrabold text-[var(--text-primary)]" aria-live="polite">
          {isLoadingSignatures
            ? <span className="inline-block h-9 w-24 rounded bg-[var(--border)] animate-pulse" />
        ...
        <div className="space-y-3.5">
          {facts.map((fact, index) => (
            <div key={index} className="flex items-start gap-3 text-left">
              <span className="mt-0.5 text-lg shrink-0 select-none">{fact.emoji}</span>
